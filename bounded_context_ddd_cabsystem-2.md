...
Tài liệu DDD này sử dụng **40 Use Case trong phạm vi hiện tại**. Mã UC được giữ theo SRS để bảo toàn traceability giữa yêu cầu, thiết kế và kiểm thử.

Các business capability chính được tổ chức thành Account/Authentication, Profile, Vehicle, Booking, Driver Matching, Assignment, Trip, Fare, Payment, Notification, Operations, Authorization, Audit và Reporting.

Mô hình dữ liệu hiện tại tập trung các entity chính trong cùng hệ thống. Thiết kế Bounded Context bên dưới **không đồng nghĩa phải giữ nguyên cách chia bảng đó**; mỗi context được phép có domain model và persistence model riêng.
...
| BC10 | Operations | Giám sát vận hành, tra cứu và theo dõi trạng thái hệ thống | UC30–UC33 |
...
# 14. BC10 — Operations Context

## 14.1 Trách nhiệm

Operations Context trả lời câu hỏi:

> "Nhân viên vận hành cần nhìn thấy và theo dõi trạng thái nghiệp vụ nào của hệ thống?"

BC10 cung cấp các **operational read models** phục vụ Operations Staff. Context này không trở thành chủ sở hữu của Customer, Driver, Vehicle, Booking hay Trip; nó chỉ cung cấp góc nhìn vận hành thông qua query model và integration data.

## 14.2 Ubiquitous Language riêng

| Thuật ngữ | Nghĩa trong BC10 |
|---|---|
| Operations View | Góc nhìn tổng hợp phục vụ nhân viên vận hành |
| Customer Operational View | Thông tin Customer cần cho việc vận hành |
| Driver Operational View | Thông tin Driver cần cho việc vận hành |
| Vehicle Operational View | Thông tin Vehicle cần cho việc vận hành |
| Trip Monitoring | Việc theo dõi trạng thái thực tế của Trip |
| Operational Status | Trạng thái được trình bày cho mục đích giám sát |
| Search | Tìm dữ liệu theo điều kiện |
| Filter | Lọc dữ liệu theo điều kiện vận hành |
| Detail | Xem chi tiết một đối tượng nghiệp vụ |

**Không gọi:** BC10 là owner của `Customer`, `Driver`, `Vehicle` hoặc `Trip`. Những object đó thuộc context nghiệp vụ gốc; BC10 chỉ sở hữu read model tương ứng.
...
Các read model này có thể được xây từ domain events, integration events hoặc query service của các context nguồn.

## 14.4 Behavior / Queries
...
ViewVehicleDetail
TrackTrip
FilterOperationalData
ViewOperationalStatus
```

## 14.5 Integration Events sử dụng
...
BC10 tiêu thụ dữ liệu từ các context khác để dựng operational view; BC10 không phát sinh lại business lifecycle của những context đó.
...
Các UC30–UC33 chủ yếu là thao tác View/Search/Filter/Detail/Status cho Operations Staff, nên trong DDD chúng nên nằm ở **query/application layer** của Operations Context thay vì cho Operations Staff truy cập trực tiếp repository của domain context khác.
...
# 16. Mapping toàn bộ 40 UC trong phạm vi hiện tại vào Bounded Context
...
| UC30 | Quản lý Customer | BC10 Operations |
| UC31 | Quản lý Driver | BC10 Operations |
| UC32 | Quản lý Vehicle | BC10 Operations |
| UC33 | Theo dõi Trip | BC10 Operations |
...
Danh sách 40 UC dưới đây là phạm vi được sử dụng cho thiết kế DDD hiện tại; mã UC được giữ nguyên để traceability.
